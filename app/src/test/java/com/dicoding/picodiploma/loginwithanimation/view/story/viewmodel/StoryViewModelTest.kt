...
//        val data: PagingData<QuoteResponseItem> = PagingData.from(dummyStory)
        val data: PagingData<ListStoryItem> = StoryPagingSource.snapshot(dummyStory)
        val expectedStory = MutableLiveData<PagingData<ListStoryItem>>()
        expectedStory.value = data
        Mockito.`when`(userRepository.getStoryPagingData()).thenReturn(expectedStory)
    ...
    fun `when Get Story Empty Should Return No Data`() = runTest {
        ...
        val mainViewModel = StoryViewModel(userRepository)
        val actualStory: PagingData<ListStoryItem> = mainViewModel.storyPagingData.getOrAwaitValue()
